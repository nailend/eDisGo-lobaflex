...
def remove_1m_lines_from_edisgo_parallel(grid_id):
    ...
    edisgo.topology.to_csv(
        os.path.join(grid_dir, str(grid_id), strategy, "topology")
    )
...
def extract_and_save_bands_parallel(grid_id):
    try:
        for use_case in ["home", "work"]:
            ...
            power.to_csv(
                data_dir + r"\{}\upper_power_{}.csv".format(grid_id, use_case)
            )
            lower.to_csv(
            ...
            print(
                "Successfully created bands for {}-{}".format(
                    grid_id, use_case
                )
            )
    except Exception:
...
def save_extreme_weeks_timeindex_no_hp(grid_id):
    ...
    if (
        extreme_week.loc[extreme_week.snapshot == max_gen.index[0]].index[0]
        < 7
    ):
        week = week - 1
        extreme_week = ts[ts.index.isocalendar().week == week].reset_index()
...
def extract_extreme_weeks_parallel(grid_id):
    ...
    if not (timeindex.isin(edisgo.timeseries.timeindex)).all():
        raise ValueError(
            "Edisgo object does not contain the given extreme weeks"
        )
    # adapt timeseries
...
def extract_extreme_weeks_ladina(
    grid_id, adapt_edisgo=False, adapt_bands=True
):
    """
    Method to get extreme weeks from previous run and reduce new objects to these weeks.
    """
    ...
    for use_case in ["home", "work"]:
        for band in bands:
            ...
            bands_dict[band].loc[timeindex].to_csv(
                os.path.join(
                    grid_dir, str(grid_id), "{}_{}.csv".format(band, use_case)
                )
            )
...
def run_feeder_extraction():
    ...
    if cpu_count > 1:
        ...
        if extract_feeders:
            ...
            pool.map_async(
                extract_feeders_parallel, grid_ids, only_flex_ev
            ).get()
